...
export function requireAllowedOrigin(request: Request) {
  ...
  if (origin) {
    return isAllowedLcvOrigin(origin) ? null : jsonResponse({ ok: false, error: 'Origem não permitida.' }, 403);
  }
...
export async function enforceRateLimit(
  ...
  if (total >= policy.maxRequests) {
    return jsonResponse({ ok: false, error: 'Muitas tentativas em pouco tempo. Tente novamente mais tarde.' }, 429, {
